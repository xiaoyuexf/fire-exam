 ...
 * --------------------------------------------------
 * 码格式：方向(1位) + 月数 + "-" + YYYYMMDD(到期日) + "-" + [随机4位] + "-" + 校验4位
 *   新码示例：M2-20261026-XK7N-193S（含 4 位随机段，每次点击都不同 → 每张码唯一）
 *   兼容旧码：M2-20261026-193S（3 段无随机，仍能通过校验，已激活的不失效）
 *   方向：M=监控(monitor)  K=维保(maintain)  B=双方向(both)
 * 校验位仅防手误/瞎填，非加密。码自带到期日，学员端本地校验，不依赖服务器。
 * 随机段与方向/月数/到期日无依赖关系，纯前端 Math.random 生成（同一参数多次生成会得到不同码）。
 * 学员端：3 段和 4 段都接受（按末尾 4 位校验），随机段不参与算法。
...
(function (root, factory) {
  ...
  function verifyCode(raw) {
    ...
    var m = code.match(/^([MKB])(\d{1,2})-(\d{8})-([0-9A-Z]{4})(?:-([0-9A-Z]{4}))?$/)
    if (!m) return { ok: false, msg: '激活码格式不正确' }
    var dir = m[1]
    var months = parseInt(m[2], 10)
    var ymd = m[3]
    var sum = m[5] || m[4] // 4 段码 m[5]=校验位；3 段码 m[5]=undefined 走 m[4]=校验位
